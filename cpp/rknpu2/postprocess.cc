...
int post_process(rknn_app_context_t *app_ctx, rknn_output *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    timer.tik();
    // Restore one binary instance mask per retained detection.  Doing this
    // before the ROI mapper is important: a class-coloured union mask would
    // merge overlapping instances and loses the segmentation semantics.
    ...
    for (int b = 0; b < boxes_num; ++b)
    {
        const float x1 = filterBoxes_by_nms[b * 4 + 0];
        const float y1 = filterBoxes_by_nms[b * 4 + 1];
        const float x2 = filterBoxes_by_nms[b * 4 + 2];
        const float y2 = filterBoxes_by_nms[b * 4 + 3];
        uint8_t *model_instance_mask = (uint8_t *)calloc(model_in_height * model_in_width, sizeof(uint8_t));
        ...
        if (!model_instance_mask || !cropped_seg_mask || !real_seg_mask)
        {
            free(model_instance_mask);
            free(cropped_seg_mask);
            free(real_seg_mask);
            continue;
        }
        for (int y = 0; y < model_in_height; ++y)
        {
            for (int x = 0; x < model_in_width; ++x)
            {
                if (x < x1 || x >= x2 || y < y1 || y >= y2)
                    continue;
#ifdef USE_FP_RESIZE
                if (seg_mask[b * model_in_width * model_in_height + y * model_in_width + x] > 0.0f)
#else
                if (seg_mask[b * model_in_width * model_in_height + y * model_in_width + x] > 0)
#endif
                    model_instance_mask[y * model_in_width + x] = 1;
            }
        }
        seg_reverse(model_instance_mask, cropped_seg_mask, real_seg_mask,
                    model_in_height, model_in_width, cropped_height, cropped_width,
                    ori_in_height, ori_in_width, y_pad, x_pad);
        od_results->results_seg[b].seg_mask = real_seg_mask;
        free(model_instance_mask);
        free(cropped_seg_mask);
    }
    free(seg_mask);
...
void release_object_detect_results(object_detect_result_list *od_results)
{
    if (od_results == NULL)
        return;
    const int count = od_results->count > OBJ_NUMB_MAX_SIZE ? OBJ_NUMB_MAX_SIZE : od_results->count;
